...
function writeI18nFiles(i18nData, config) {
  const outputDir = config.filePaths?.outputDir || './i18n-output';
...
function createObjectExpression(obj) {
  logger.info('创建对象表达式:', obj);
  const properties = Object.entries(obj).map(([key, value]) => {
    // 使用stringLiteral而不是identifier来处理所有键，包括以数字开头的键
    const keyLiteral = t.stringLiteral(key.trim());
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      // 字符串值
      // 检查value是否为null或undefined
      if (value === null || value === undefined) {
        logger.warn(`值为null或undefined，使用空字符串代替: ${key}`);
        value = '';
      }
      return t.objectProperty(
        keyLiteral,
